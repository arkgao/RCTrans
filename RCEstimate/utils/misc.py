...
def cp_options(opt, path):
    """Automatically copy the option file to the experiments/options folder.

    Rename the old option with 'archive' suffix.

    Args:
        opt (dict): options
        path (str): the path of option file
    """
    path = os.path.abspath(path)
    path_opt = opt['path'].get('options', None)
    if path_opt is not None:
        if opt['is_train']:
            base = osp.basename(path)
            new_path = osp.join(path_opt, base)
            if osp.exists(new_path):
                new_name = base.split('.')[0] + '_archived_' + get_time_str() + '.yml'
                tmp_path = osp.join(path_opt, new_name)
                print(f'Option path already exists. Rename it to {new_name}', flush=True)
...
def make_exp_dirs(opt):
    ...
    for key, path in path_opt.items():
        if key in ['root', 'experiments_root', 'models', 'training_states', 'log', 'visualization', 'options']:
            os.makedirs(path, exist_ok=True)
